# InvoicingFramework

Defines the invoicing framework.  - invoicing_framework_code: Chorus Pro code (A1, A2, A9, A12) - used for B2G - operation_nature: Operation nature (B1, S1, M1, etc.) - priority for Factur-X  If operation_nature is provided, it will be used directly in Factur-X XML (BT-23). Otherwise, the code will be derived from invoicing_framework_code via automatic mapping.  Example:     >>> framework = InvoicingFramework(     ...     invoicing_framework_code=InvoicingFrameworkCode.A1_SUPPLIER_INVOICE,     ...     operation_nature=OperationNature.GOODS  # Forces B1 instead of S1     ... )

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**invoicing_framework_code** | [**InvoicingFrameworkCode**](InvoicingFrameworkCode.md) | Chorus Pro framework code (A1, A2, A9, A12) | 
**operation_nature** | [**OperationNature**](OperationNature.md) |  | [optional] 
**approver_service_code** | **str** |  | [optional] 
**approver_structure_code** | **str** |  | [optional] 

## Example

```python
from factpulse.models.invoicing_framework import InvoicingFramework

# TODO update the JSON string below
json = "{}"
# create an instance of InvoicingFramework from a JSON string
invoicing_framework_instance = InvoicingFramework.from_json(json)
# print the JSON string representation of the object
print(InvoicingFramework.to_json())

# convert the object into a dict
invoicing_framework_dict = invoicing_framework_instance.to_dict()
# create an instance of InvoicingFramework from a dict
invoicing_framework_from_dict = InvoicingFramework.from_dict(invoicing_framework_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


